...
public class CloudSettingsHelper {
    ...
    private void syncToCloud(Utilities.Callback2<Boolean, String> callback) {
        String rawConfig = LeleConfig.exportConfigs();
        String compressed = encodeConfig(rawConfig);
        String payload = rawConfig.length() >= compressed.length() ? compressed : rawConfig;
        getCloudStorageHelper().setItem(CLOUD_SETTINGS_KEY, payload, (res, error) -> {
            if (error == null) {
            ...
            if (error == null) {
                if (TextUtils.isEmpty(res)) {
                    ...
                    if (config == null) {
                        ...
                        try {
                            LeleConfig.importConfigs(config);
