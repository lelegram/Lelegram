...
import com.fylnx.lelegram.helpers.VoiceEnhancementsHelper;
import com.fylnx.lelegram.helpers.WhisperHelper;

public class LeleChatSettingsActivity extends BaseLeleSettingsActivity implements NotificationCenter.NotificationCenterDelegate {
    ...
    private final int hideChannelAdsRow = rowId++;
    ...
    private final int hideAudioVideoButtonRow = rowId++;
    ...
    public View createView(Context context) {
        ...
        resetItem.setOnClickListener(v -> {
            ...
            if (stickerCell != null) {
                ValueAnimator animator = ValueAnimator.ofFloat(LeleConfig.stickerSize, 14.0f);
                animator.setDuration(150);
                animator.setInterpolator(CubicBezierInterpolator.EASE_OUT_QUINT);
                animator.addUpdateListener(valueAnimator -> {
                    var floatValue = (float) valueAnimator.getAnimatedValue();
                    LeleConfig.setStickerSize(floatValue);
                    stickerCell.setValue(floatValue);
                });
                animator.start();
            } else {
                LeleConfig.setStickerSize(14.0f);
            }
            item.floatValue = 14.0f;
        });
        AndroidUtilities.updateViewVisibilityAnimated(resetItem, Float.compare(LeleConfig.stickerSize, 14.0f) != 0, 1f, false);
    ...
    public String getDoubleTapActionText(int action) {
        return switch (action) {
            case LeleConfig.DOUBLE_TAP_ACTION_REACTION ->
                    LocaleController.getString(R.string.Reactions);
            case LeleConfig.DOUBLE_TAP_ACTION_TRANSLATE ->
                    LocaleController.getString(R.string.TranslateMessage);
            case LeleConfig.DOUBLE_TAP_ACTION_REPLY -> LocaleController.getString(R.string.Reply);
            case LeleConfig.DOUBLE_TAP_ACTION_SAVE ->
                    LocaleController.getString(R.string.AddToSavedMessages);
            case LeleConfig.DOUBLE_TAP_ACTION_REPEAT -> LocaleController.getString(R.string.Repeat);
            case LeleConfig.DOUBLE_TAP_ACTION_EDIT -> LocaleController.getString(R.string.Edit);
    ...
    protected void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        items.add(StickerSizeCellFactory.of(stickerSizeRow, LocaleController.getString(R.string.StickerSize), LeleConfig.stickerSize, progress -> {
            LeleConfig.setStickerSize(progress);
        ...
        items.add(UItem.asCheck(hideTimeOnStickerRow, LocaleController.getString(R.string.HideTimeOnSticker)).slug("hideTimeOnSticker").setChecked(LeleConfig.hideTimeOnSticker));
        items.add(UItem.asCheck(showTimeHintRow, LocaleController.getString(R.string.ShowTimeHint), LocaleController.getString(R.string.ShowTimeHintDesc)).slug("showTimeHint").setChecked(LeleConfig.showTimeHint));
        items.add(UItem.asCheck(reducedColorsRow, LocaleController.getString(R.string.ReducedColors)).slug("reducedColors").setChecked(LeleConfig.reducedColors));
        ...
        items.add(UItem.asCheck(disableJumpToNextRow, LocaleController.getString(R.string.DisableJumpToNextChannel)).slug("disableJumpToNext").setChecked(LeleConfig.disableJumpToNextChannel));
        items.add(UItem.asCheck(disableGreetingStickerRow, LocaleController.getString(R.string.DisableGreetingSticker)).slug("disableGreetingSticker").setChecked(LeleConfig.disableGreetingSticker));
        items.add(UItem.asCheck(hideChannelBottomButtonsRow, LocaleController.getString(R.string.HideChannelBottomButtons)).slug("hideChannelBottomButtons").setChecked(LeleConfig.hideChannelBottomButtons));
        items.add(UItem.asCheck(hideChannelAdsRow, LocaleController.getString(R.string.BlockChannelAds)).slug("hideChannelAds").setChecked(LeleConfig.hideChannelAds));
        items.add(TextSettingsCellFactory.of(doubleTapActionRow, LocaleController.getString(R.string.DoubleTapAction), LeleConfig.doubleTapInAction == LeleConfig.doubleTapOutAction ?
                getDoubleTapActionText(LeleConfig.doubleTapInAction) :
                getDoubleTapActionText(LeleConfig.doubleTapInAction) + ", " + getDoubleTapActionText(LeleConfig.doubleTapOutAction)).slug("doubleTapAction"));
        items.add(TextSettingsCellFactory.of(maxRecentStickersRow, LocaleController.getString(R.string.MaxRecentStickers), String.valueOf(LeleConfig.maxRecentStickers)).slug("maxRecentStickers"));
        items.add(UItem.asShadow(null));

        items.add(UItem.asHeader(LocaleController.getString(R.string.PremiumPreviewVoiceToText)));
        items.add(TextSettingsCellFactory.of(transcribeProviderRow, LocaleController.getString(R.string.TranscribeProviderShort), switch (LeleConfig.transcribeProvider) {
            case LeleConfig.TRANSCRIBE_AUTO ->
                    LocaleController.getString(R.string.TranscribeProviderAuto);
            case LeleConfig.TRANSCRIBE_WORKERSAI ->
        ...
        items.add(UItem.asCheck(markdownEnableRow, LocaleController.getString(R.string.MarkdownEnableByDefault)).slug("markdownEnable").setChecked(!LeleConfig.disableMarkdownByDefault));
        items.add(TextSettingsCellFactory.of(markdownParserRow, LocaleController.getString(R.string.MarkdownParser), LeleConfig.newMarkdownParser ? "Lelegram" : "Telegram").slug("markdownParser"));
        if (LeleConfig.newMarkdownParser) {
            items.add(UItem.asCheck(markdownParseLinksRow, LocaleController.getString(R.string.MarkdownParseLinks)).slug("markdownParseLinks").setChecked(LeleConfig.markdownParseLinks));
        }
        items.add(UItem.asShadow(markdown2Row, TextUtils.expandTemplate(EntitiesHelper.parseMarkdown(LeleConfig.newMarkdownParser && LeleConfig.markdownParseLinks ? LocaleController.getString(R.string.MarkdownAbout) : LocaleController.getString(R.string.MarkdownAbout2)), "**", "__", "~~", "`", "||", "[", "](", ")")));

        items.add(UItem.asHeader(LocaleController.getString(R.string.SharedMediaTab2)));
        if (VoiceEnhancementsHelper.isAvailable()) {
            items.add(UItem.asCheck(voiceEnhancementsRow, LocaleController.getString(R.string.VoiceEnhancements), LocaleController.getString(R.string.VoiceEnhancementsAbout)).slug("voiceEnhancements").setChecked(LeleConfig.voiceEnhancements));
        }
        items.add(UItem.asCheck(rearVideoMessagesRow, LocaleController.getString(R.string.RearVideoMessages)).slug("rearVideoMessages").setChecked(LeleConfig.rearVideoMessages));
        items.add(UItem.asCheck(confirmAVRow, LocaleController.getString(R.string.ConfirmAVMessage)).slug("confirmAV").setChecked(LeleConfig.confirmAVMessage));
        items.add(UItem.asCheck(hideAudioVideoButtonRow, LocaleController.getString(R.string.HideAudioVideoButton)).slug("hideAudioVideoButton").setChecked(LeleConfig.hideAudioVideoButton));
        items.add(UItem.asCheck(disableProximityEventsRow, LocaleController.getString(R.string.DisableProximityEvents)).slug("disableProximityEvents").setChecked(LeleConfig.disableProximityEvents));
        items.add(UItem.asCheck(disableVoiceMessageAutoPlayRow, LocaleController.getString(R.string.DisableVoiceMessagesAutoPlay)).slug("disableVoiceMessageAutoPlay").setChecked(LeleConfig.disableVoiceMessageAutoPlay));
        items.add(UItem.asCheck(unmuteVideosWithVolumeButtonsRow, LocaleController.getString(R.string.UnmuteVideosWithVolumeButtons)).slug("unmuteVideosWithVolumeButtons").setChecked(LeleConfig.unmuteVideosWithVolumeButtons));
        items.add(UItem.asCheck(autoPauseVideoRow, LocaleController.getString(R.string.AutoPauseVideo), LocaleController.getString(R.string.AutoPauseVideoAbout)).slug("autoPauseVideo").setChecked(LeleConfig.autoPauseVideo));
        items.add(UItem.asCheck(preferOriginalQualityRow, LocaleController.getString(R.string.PreferOriginalQuality), LocaleController.getString(R.string.PreferOriginalQualityDesc)).slug("preferOriginalQuality").setChecked(LeleConfig.preferOriginalQuality));
        items.add(UItem.asShadow(null));

        items.add(UItem.asHeader(LocaleController.getString(R.string.MessageMenu)));
        items.add(TextCheckbox2CellFactory.of(messageMenuRow + 1, LocaleController.getString(R.string.DeleteDownloadedFile)).slug("showDeleteDownloadedFile").setChecked(LeleConfig.showDeleteDownloadedFile));
        items.add(TextCheckbox2CellFactory.of(messageMenuRow + 2, LocaleController.getString(R.string.NoQuoteForward)).slug("showNoQuoteForward").setChecked(LeleConfig.showNoQuoteForward));
        items.add(TextCheckbox2CellFactory.of(messageMenuRow + 3, LocaleController.getString(R.string.AddToSavedMessages)).slug("showAddToSavedMessages").setChecked(LeleConfig.showAddToSavedMessages));
        items.add(TextCheckbox2CellFactory.of(messageMenuRow + 4, LocaleController.getString(R.string.Repeat)).slug("showRepeat").setChecked(LeleConfig.showRepeat));
        items.add(TextCheckbox2CellFactory.of(messageMenuRow + 5, LocaleController.getString(R.string.Prpr)).slug("showPrPr").setChecked(LeleConfig.showPrPr));
        items.add(TextCheckbox2CellFactory.of(messageMenuRow + 6, LocaleController.getString(R.string.TranslateMessage)).slug("showTranslate").setChecked(LeleConfig.showTranslate));
        items.add(TextCheckbox2CellFactory.of(messageMenuRow + 7, LocaleController.getString(R.string.ReportChat)).slug("showReport").setChecked(LeleConfig.showReport));
        items.add(TextCheckbox2CellFactory.of(messageMenuRow + 8, LocaleController.getString(R.string.MessageDetails)).slug("showMessageDetails").setChecked(LeleConfig.showMessageDetails));
        items.add(TextCheckbox2CellFactory.of(messageMenuRow + 9, LocaleController.getString(R.string.ViewHistory)).slug("showMessageEditHistory").setChecked(LeleConfig.showMessageEditHistory));
        items.add(TextCheckbox2CellFactory.of(messageMenuRow + 10, LocaleController.getString(R.string.CopyPhoto)).slug("showCopyPhoto").setChecked(LeleConfig.showCopyPhoto));
        items.add(TextCheckbox2CellFactory.of(messageMenuRow + 11, LocaleController.getString(R.string.SetReminder)).slug("showSetReminder").setChecked(LeleConfig.showSetReminder));
        items.add(TextCheckbox2CellFactory.of(messageMenuRow + 12, LocaleController.getString(R.string.QrCode)).slug("showQrCode").setChecked(LeleConfig.showQrCode));
        items.add(TextCheckbox2CellFactory.of(messageMenuRow + 13, LocaleController.getString(R.string.OpenInExternalApp)).slug("showOpenIn").setChecked(LeleConfig.showOpenIn));
        items.add(UItem.asShadow(null));
    }
    ...
    protected void onItemClick(UItem item, View view, int position, float x, float y) {
        ...
        if (id == ignoreBlockedRow) {
            LeleConfig.toggleIgnoreBlocked();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.ignoreBlocked);
            }
        } else if (id == keepDeletedMessagesRow) {
            ...
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.allowProtectedContentActions);
            }
        } else if (id == hideKeyboardOnChatScrollRow) {
            LeleConfig.toggleHideKeyboardOnChatScroll();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.hideKeyboardOnChatScroll);
            }
        } else if (id == rearVideoMessagesRow) {
            LeleConfig.toggleRearVideoMessages();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.rearVideoMessages);
            }
        } else if (id == confirmAVRow) {
            LeleConfig.toggleConfirmAVMessage();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.confirmAVMessage);
            }
        } else if (id == hideAudioVideoButtonRow) {
            LeleConfig.toggleHideAudioVideoButton();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.hideAudioVideoButton);
            }
            parentLayout.rebuildAllFragmentViews(false, false);
        } else if (id == disableProximityEventsRow) {
            LeleConfig.toggleDisableProximityEvents();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.disableProximityEvents);
            }
            showRestartBulletin();
        } else if (id == tryToOpenAllLinksInIVRow) {
            LeleConfig.toggleTryToOpenAllLinksInIV();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.tryToOpenAllLinksInIV);
            }
        } else if (id == autoPauseVideoRow) {
            LeleConfig.toggleAutoPauseVideo();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.autoPauseVideo);
            }
        } else if (id == disableJumpToNextRow) {
            LeleConfig.toggleDisableJumpToNextChannel();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.disableJumpToNextChannel);
            }
        } else if (id == disableGreetingStickerRow) {
            LeleConfig.toggleDisableGreetingSticker();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.disableGreetingSticker);
            }
        } else if (id == disableVoiceMessageAutoPlayRow) {
            LeleConfig.toggleDisableVoiceMessageAutoPlay();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.disableVoiceMessageAutoPlay);
            }
        } else if (id == unmuteVideosWithVolumeButtonsRow) {
            LeleConfig.toggleUnmuteVideosWithVolumeButtons();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.unmuteVideosWithVolumeButtons);
            }
        } else if (id == doubleTapActionRow) {
            ArrayList<String> arrayList = new ArrayList<>();
            ArrayList<Integer> types = new ArrayList<>();
            arrayList.add(LocaleController.getString(R.string.Disable));
            types.add(LeleConfig.DOUBLE_TAP_ACTION_NONE);
            arrayList.add(LocaleController.getString(R.string.Reactions));
            types.add(LeleConfig.DOUBLE_TAP_ACTION_REACTION);
            arrayList.add(LocaleController.getString(R.string.TranslateMessage));
            types.add(LeleConfig.DOUBLE_TAP_ACTION_TRANSLATE);
            arrayList.add(LocaleController.getString(R.string.Reply));
            types.add(LeleConfig.DOUBLE_TAP_ACTION_REPLY);
            arrayList.add(LocaleController.getString(R.string.AddToSavedMessages));
            types.add(LeleConfig.DOUBLE_TAP_ACTION_SAVE);
            arrayList.add(LocaleController.getString(R.string.Repeat));
            types.add(LeleConfig.DOUBLE_TAP_ACTION_REPEAT);
            arrayList.add(LocaleController.getString(R.string.Edit));
            types.add(LeleConfig.DOUBLE_TAP_ACTION_EDIT);
            ...
            for (int i = 0; i < 2; i++) {
                ...
                for (int a = 0; a < arrayList.size(); a++) {
                    ...
                    cell.setTag(a);
                    cell.setTextAndValue(arrayList.get(a), a == types.indexOf(out ? LeleConfig.doubleTapOutAction : LeleConfig.doubleTapInAction));
                    ...
                    cell.setOnClickListener(v -> {
                        ...
                        var old = out ? LeleConfig.doubleTapOutAction : LeleConfig.doubleTapInAction;
                        if (types.get(which) == old) {
                            return;
                        }
                        if (out) {
                            LeleConfig.setDoubleTapOutAction(types.get(which));
                        } else {
                            LeleConfig.setDoubleTapInAction(types.get(which));
                        }
                        ((RadioColorCell) layout.getChildAt(types.indexOf(old))).setChecked(false, true);
                        cell.setChecked(true, true);
                        item.textValue = LeleConfig.doubleTapInAction == LeleConfig.doubleTapOutAction ?
                                getDoubleTapActionText(LeleConfig.doubleTapInAction) :
                                getDoubleTapActionText(LeleConfig.doubleTapInAction) + ", " + getDoubleTapActionText(LeleConfig.doubleTapOutAction);
            ...
            LeleConfig.toggleDisableMarkdownByDefault();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(!LeleConfig.disableMarkdownByDefault);
            }
        } else if (id > messageMenuRow) {
            TextCheckbox2Cell cell = ((TextCheckbox2Cell) view);
            int menuPosition = id - messageMenuRow - 1;
            if (menuPosition == 0) {
                LeleConfig.toggleShowDeleteDownloadedFile();
                cell.setChecked(LeleConfig.showDeleteDownloadedFile);
            } else if (menuPosition == 1) {
                LeleConfig.toggleShowNoQuoteForward();
                cell.setChecked(LeleConfig.showNoQuoteForward);
            } else if (menuPosition == 2) {
                LeleConfig.toggleShowAddToSavedMessages();
                cell.setChecked(LeleConfig.showAddToSavedMessages);
            } else if (menuPosition == 3) {
                LeleConfig.toggleShowRepeat();
                cell.setChecked(LeleConfig.showRepeat);
            } else if (menuPosition == 4) {
                LeleConfig.toggleShowPrPr();
                cell.setChecked(LeleConfig.showPrPr);
            } else if (menuPosition == 5) {
                LeleConfig.toggleShowTranslate();
                cell.setChecked(LeleConfig.showTranslate);
            } else if (menuPosition == 6) {
                LeleConfig.toggleShowReport();
                cell.setChecked(LeleConfig.showReport);
            } else if (menuPosition == 7) {
                LeleConfig.toggleShowMessageDetails();
                cell.setChecked(LeleConfig.showMessageDetails);
            } else if (menuPosition == 8) {
                LeleConfig.toggleShowMessageEditHistory();
                cell.setChecked(LeleConfig.showMessageEditHistory);
            } else if (menuPosition == 9) {
                LeleConfig.toggleShowCopyPhoto();
                cell.setChecked(LeleConfig.showCopyPhoto);
            } else if (menuPosition == 10) {
                LeleConfig.toggleShowSetReminder();
                cell.setChecked(LeleConfig.showSetReminder);
            } else if (menuPosition == 11) {
                LeleConfig.toggleShowQrCode();
                cell.setChecked(LeleConfig.showQrCode);
            } else if (menuPosition == 12) {
                LeleConfig.toggleShowOpenIn();
                cell.setChecked(LeleConfig.showOpenIn);
            }
        } else if (id == voiceEnhancementsRow) {
            LeleConfig.toggleVoiceEnhancements();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.voiceEnhancements);
            }
            ...
            PopupHelper.show(types, LocaleController.getString(R.string.MaxRecentStickers), types.indexOf(String.valueOf(LeleConfig.maxRecentStickers)), getParentActivity(), view, i -> {
                LeleConfig.setMaxRecentStickers(Integer.parseInt(types.get(i)));
                item.textValue = String.valueOf(LeleConfig.maxRecentStickers);
                listView.adapter.notifyItemChanged(position, PARTIAL);
            }, resourcesProvider);
        } else if (id == hideTimeOnStickerRow) {
            LeleConfig.toggleHideTimeOnSticker();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.hideTimeOnSticker);
            }
            ...
            ArrayList<String> arrayList = new ArrayList<>();
            arrayList.add("Lelegram");
            arrayList.add("Telegram");
            boolean oldParser = LeleConfig.newMarkdownParser;
            PopupHelper.show(arrayList, LocaleController.getString(R.string.MarkdownParser), LeleConfig.newMarkdownParser ? 0 : 1, getParentActivity(), view, i -> {
                LeleConfig.setNewMarkdownParser(i == 0);
                item.textValue = arrayList.get(i);
                listView.adapter.notifyItemChanged(position, PARTIAL);
                if (oldParser != LeleConfig.newMarkdownParser) {
                    if (oldParser) {
            ...
            LeleConfig.toggleMarkdownParseLinks();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.markdownParseLinks);
            }
            notifyItemChanged(markdown2Row);
        } else if (id == quickForwardRow) {
            LeleConfig.toggleQuickForward();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.quickForward);
            }
        } else if (id == reducedColorsRow) {
            LeleConfig.toggleReducedColors();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.reducedColors);
            }
            var stickerCell = listView.findViewByItemId(stickerSizeRow);
            stickerCell.invalidate();
        } else if (id == showTimeHintRow) {
            LeleConfig.toggleShowTimeHint();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.showTimeHint);
            }
        } else if (id == transcribeProviderRow) {
            ArrayList<String> arrayList = new ArrayList<>();
            ArrayList<Integer> types = new ArrayList<>();
            arrayList.add(LocaleController.getString(R.string.TranscribeProviderAuto));
            types.add(LeleConfig.TRANSCRIBE_AUTO);
            arrayList.add(LocaleController.getString(R.string.TelegramPremium));
            types.add(LeleConfig.TRANSCRIBE_PREMIUM);
            arrayList.add(LocaleController.getString(R.string.TranscribeProviderWorkersAI));
            types.add(LeleConfig.TRANSCRIBE_WORKERSAI);
            PopupHelper.show(arrayList, LocaleController.getString(R.string.TranscribeProviderShort), types.indexOf(LeleConfig.transcribeProvider), getParentActivity(), view, i -> {
                LeleConfig.setTranscribeProvider(types.get(i));
                item.textValue = arrayList.get(i);
                listView.adapter.notifyItemChanged(position, PARTIAL);
            }, resourcesProvider);
            ...
            LeleConfig.togglePreferOriginalQuality();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.preferOriginalQuality);
            }
        } else if (id == hideChannelBottomButtonsRow) {
            LeleConfig.toggleHideChannelBottomButtons();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.hideChannelBottomButtons);
            }
        } else if (id == hideChannelAdsRow) {
            LeleConfig.toggleHideChannelAds();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.hideChannelAds);
            }
        }
    }
    ...
    private static class StickerSizeCell extends FrameLayout {
        ...
        public StickerSizeCell(Context context, Theme.ResourcesProvider resourcesProvider) {
            ...
            sizeBar.setValue(LeleConfig.stickerSize);
