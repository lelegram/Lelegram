...
public class LeleGeneralSettingsActivity extends BaseLeleSettingsActivity {
    ...
    private final int hideAttachCameraBlockRow = rowId++;
    ...
    private CharSequence getTranslationProvider() {
        ...
        int index = types.indexOf(LeleConfig.translationProvider);
        if (index < 0) {
            return "";
        } else {
    ...
    private CharSequence getTranslationTarget() {
        var language = LeleConfig.translationTarget;
        CharSequence value;
    ...
    private CharSequence getTranslatorType() {
        return switch (LeleConfig.transType) {
            case LeleConfig.TRANS_TYPE_TG -> LocaleController.getString(R.string.TranslatorTypeTG);
            case LeleConfig.TRANS_TYPE_EXTERNAL ->
                    LocaleController.getString(R.string.TranslatorTypeExternal);
            default -> LocaleController.getString(R.string.TranslatorTypeLele);
        };
    }
    ...
    protected void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        ...
        items.add(UItem.asCheck(ipv6Row, LocaleController.getString(R.string.PreferIPv6)).slug("ipv6").setChecked(LeleConfig.preferIPv6));
        ...
        if (LeleConfig.transType != LeleConfig.TRANS_TYPE_EXTERNAL) {
            if (LeleConfig.transType == LeleConfig.TRANS_TYPE_LELE) {
                items.add(UItem.asCheck(showOriginalRow, LocaleController.getString(R.string.TranslatorShowOriginal)).slug("showOriginalRow").setChecked(LeleConfig.showOriginal));
            }
            ...
            items.add(UItem.asCheck(autoTranslateRow, LocaleController.getString(R.string.AutoTranslate), LocaleController.getString(R.string.AutoTranslateAbout)).slug("autoTranslate").setChecked(LeleConfig.autoTranslate));
        } else {
        ...
        items.add(UItem.asCheck(accentAsNotificationColorRow, LocaleController.getString(R.string.AccentAsNotificationColor)).slug("accentAsNotificationColor").setChecked(LeleConfig.accentAsNotificationColor));
        items.add(UItem.asCheck(silenceNonContactsRow, LocaleController.getString(R.string.SilenceNonContacts)).slug("silenceNonContacts").setChecked(LeleConfig.silenceNonContacts));
        ...
        items.add(TextSettingsCellFactory.of(nameOrderRow, LocaleController.getString(R.string.NameOrder), switch (LeleConfig.nameOrder) {
            case 2 -> LocaleController.getString(R.string.LastFirst);
            default -> LocaleController.getString(R.string.FirstLast);
        }).slug("nameOrder"));
        items.add(TextSettingsCellFactory.of(idTypeRow, LocaleController.getString(R.string.IdType), switch (LeleConfig.idType) {
            case LeleConfig.ID_TYPE_HIDDEN -> LocaleController.getString(R.string.IdTypeHidden);
            case LeleConfig.ID_TYPE_BOTAPI -> LocaleController.getString(R.string.IdTypeBOTAPI);
        ...
        items.add(UItem.asCheck(disabledInstantCameraRow, LocaleController.getString(R.string.DisableInstantCamera)).slug("disabledInstantCamera").setChecked(LeleConfig.disableInstantCamera));
        items.add(UItem.asCheck(hideAttachCameraBlockRow, LocaleController.getString(R.string.HideAttachCameraBlock)).slug("hideAttachCameraBlock").setChecked(LeleConfig.hideAttachCameraBlock));
        items.add(UItem.asCheck(askBeforeCallRow, LocaleController.getString(R.string.AskBeforeCalling)).slug("askBeforeCall").setChecked(LeleConfig.askBeforeCall));
        items.add(UItem.asCheck(openArchiveOnPullRow, LocaleController.getString(R.string.OpenArchiveOnPull)).slug("openArchiveOnPull").setChecked(LeleConfig.openArchiveOnPull));
        items.add(UItem.asShadow(null));
    }
    ...
    protected void onItemClick(UItem item, View view, int position, float x, float y) {
        var id = item.id;
        if (id == ipv6Row) {
            LeleConfig.toggleIPv6();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.preferIPv6);
            }
            ...
            LeleConfig.toggleDisabledInstantCamera();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.disableInstantCamera);
            }
        } else if (id == hideAttachCameraBlockRow) {
            LeleConfig.toggleHideAttachCameraBlock();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.hideAttachCameraBlock);
            }
            ...
            types.add(2);
            PopupHelper.show(arrayList, LocaleController.getString(R.string.NameOrder), types.indexOf(LeleConfig.nameOrder), getParentActivity(), view, i -> {
                LeleConfig.setNameOrder(types.get(i));
            ...
            LeleConfig.toggleOpenArchiveOnPull();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.openArchiveOnPull);
            }
        } else if (id == askBeforeCallRow) {
            LeleConfig.toggleAskBeforeCall();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.askBeforeCall);
            }
        } else if (id == idTypeRow) {
            ArrayList<String> arrayList = new ArrayList<>();
            ArrayList<Integer> types = new ArrayList<>();
            arrayList.add(LocaleController.getString(R.string.IdTypeHidden));
            types.add(LeleConfig.ID_TYPE_HIDDEN);
            arrayList.add(LocaleController.getString(R.string.IdTypeAPI));
            types.add(LeleConfig.ID_TYPE_API);
            arrayList.add(LocaleController.getString(R.string.IdTypeBOTAPI));
            types.add(LeleConfig.ID_TYPE_BOTAPI);
            PopupHelper.show(arrayList, LocaleController.getString(R.string.IdType), types.indexOf(LeleConfig.idType), getParentActivity(), view, i -> {
                LeleConfig.setIdType(types.get(i));
                item.textValue = arrayList.get(i);
                listView.adapter.notifyItemChanged(position, PARTIAL);
                parentLayout.rebuildAllFragmentViews(false, false);
            }, resourcesProvider);
        } else if (id == accentAsNotificationColorRow) {
            LeleConfig.toggleAccentAsNotificationColor();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.accentAsNotificationColor);
            }
        } else if (id == silenceNonContactsRow) {
            LeleConfig.toggleSilenceNonContacts();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.silenceNonContacts);
            }
        } else if (id == translatorTypeRow) {
            int oldType = LeleConfig.transType;
            Translator.showTranslatorTypeSelector(getParentActivity(), view, () -> {
                int newType = LeleConfig.transType;
                item.textValue = getTranslatorType();
                listView.adapter.notifyItemChanged(position, PARTIAL);
                if (oldType != newType) {
                    int count = 4;
                    if (oldType == LeleConfig.TRANS_TYPE_LELE || newType == LeleConfig.TRANS_TYPE_LELE) {
                        count++;
                    }
                    if (oldType == LeleConfig.TRANS_TYPE_EXTERNAL) {
                        notifyItemRemoved(translatorExternalAppRow);
                        updateRows();
                        notifyItemRangeInserted(translationProviderRow, count);
                    } else if (newType == LeleConfig.TRANS_TYPE_EXTERNAL) {
                        notifyItemRangeRemoved(translationProviderRow, count);
                        updateRows();
                        notifyItemInserted(translatorExternalAppRow);
                    } else if (oldType == LeleConfig.TRANS_TYPE_LELE) {
                        notifyItemRemoved(showOriginalRow);
                        updateRows();
                    } else if (newType == LeleConfig.TRANS_TYPE_LELE) {
                        updateRows();
            ...
            presentFragment(new LeleLanguagesSelectActivity(LeleLanguagesSelectActivity.TYPE_RESTRICTED));
        } else if (id == autoTranslateRow) {
            LeleConfig.toggleAutoTranslate();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.autoTranslate);
            }
        } else if (id == showOriginalRow) {
            LeleConfig.toggleShowOriginal();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(LeleConfig.showOriginal);
            }
